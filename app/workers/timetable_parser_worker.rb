...
#       inserts.push "(#{ c.id }, #{ t.id }, #{ schedule.group }, #{ schedule.room }, #{ schedule.professor }, #{ DateTime.new(schedule.startDate.getYear, schedule.startDate.getMonthOfYear, schedule.startDate.getDayOfMonth, schedule.startDate.getHourOfDay, schedule.startDate.getMinuteOfHour) }, #{ DateTime.new(schedule.endDate.getYear, schedule.endDate.getMonthOfYear, schedule.endDate.getDayOfMonth, schedule.endDate.getHourOfDay, schedule.endDate.getMinuteOfHour) }, #{ now })"
...
#     sql = "INSERT INTO schedules (`course_id`, `timetable_id`, `group`, `room`, `professor`, `start_datetime`, `end_datetime`, `updated_at`) VALUES #{ inserts.join(", ") }"
